...
class OpenSimplex2F(seed: Long) {
    ...
    private val permGrad2: Array<Grad2?> = arrayOfNulls(PSIZE)
    private val permGrad3: Array<Grad3?> = arrayOfNulls(PSIZE)
    private val permGrad4: Array<Grad4?> = arrayOfNulls(PSIZE)
    /*
	 * Noise Evaluators
	 */
    /**
    ...
    private fun noise4_Base(xs: Double, ys: Double, zs: Double, ws: Double): Double {
        ...
        val aabbScore = abs(aabb)
        val ababScore = abs(abab)
        val abbaScore = abs(abba)
	 ...
	 */
        ...
        private val VERTICES_4D: Array<LatticePoint4D?>
        ...
        init {
            LOOKUP_2D = arrayOfNulls(4)
            LOOKUP_3D = arrayOfNulls(8)
            VERTICES_4D = arrayOfNulls(16)
            ...
            for (i in 0..15) {
                VERTICES_4D[i] =
                    LatticePoint4D(i shr 0 and 1, i shr 1 and 1, i shr 2 and 1, i shr 3 and 1)
            }
        }
    ...
    init {
        ...
        for (i in PSIZE - 1 downTo 0) {
            val tseed = seed * 6364136223846793005L + 1442695040888963407L
            var r = ((tseed + 31) % (i + 1)).toInt()
            if (r < 0) r += i + 1
            perm[i] = source[r]
